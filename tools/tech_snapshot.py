    ...
    try:
        ...
        for i, t in enumerate(ts):
            ...
            rows.append({
                ...
                "close": c,
                "adjclose": (adj or [None] * len(ts))[i] if adj else c,
...
def build(ticker, benchmark="SPY"):
    ...
    b = fetch_ohlcv(benchmark)
    bcl = [r["adjclose"] for r in b] if b else []
    rs = {}
    for label, n in (("1M", 21), ("3M", 63), ("6M", 126)):
        ps, pb = perf(closes, n), (perf(bcl, n) if bcl else None)
        rs[label] = {"stock": ps, "bench": pb,
                     "rel": (ps - pb) if (ps is not None and pb is not None) else None}
...
def render(s):
    ...
    L.append("| 项 | 值 |")
    L.append("|---|---|")
    L.append(f"| 最新收盘（复权） | {fmt(s['price'], ' USD')} |")
    ...
    lv = s["levels"]
    L.append(f"- **上方阻力**：{', '.join(fmt(x) for x in lv['resistance']) or '近 180 日内无更高摆动高点'}")
    L.append(f"- **下方支撑**：{', '.join(fmt(x) for x in lv['support']) or '近 180 日内无更低摆动低点'}")
    L.append("\n> 这些是过去 180 日内的局部极值（前后各 5 根确认），**不是**「历史验证过会反弹的位置」。"
             "不要把它们描述成有预测力的位置。\n")
